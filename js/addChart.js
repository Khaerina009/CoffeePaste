document.addEventListener("DOMContentLoaded", function () {
  ...
  orderNowButton.addEventListener("click", function () {
    ...
    Object.entries(cartItems).forEach(
      ([productName, { quantity, totalPrice, image }]) => {
        const inputQuantity = document.createElement("input");
        inputQuantity.type = "hidden";
        inputQuantity.name = `products[${encodeURIComponent(
          productName
        )}][quantity]`;
        ...
        inputTotalPrice.name = `products[${encodeURIComponent(
          productName
        )}][totalPrice]`;
        ...
        inputImage.name = `products[${encodeURIComponent(
          productName
        )}][image]`;
  ...
  function renderCart() {
    ...
    for (let item in cartItems) {
      if (cartItems[item].quantity > 0) {
        ...
        hargaText.innerHTML = `Rp ${cartItems[item].totalPrice.toLocaleString(
          "id-ID"
        )}`;
    ...
    totalHargaElement.innerHTML = `TOTAL: <span class="totalListCart"> Rp ${totalHarga.toLocaleString(
      "id-ID"
    )}</span>`;
  ...
  function decreaseQuantity(productName) {
      const hargaProdukPerItem =
        cartItems[productName].totalPrice / cartItems[productName].quantity;
  ...
  addToCartButtons.forEach(function (button) {
    button.addEventListener("click", function (event) {
      ...
      const productPrice = parseFloat(
        productContainer.querySelector("span").textContent
          .replace("Rp ", "")
          .replace(".", "")
          .replace(",", ".")
      );
